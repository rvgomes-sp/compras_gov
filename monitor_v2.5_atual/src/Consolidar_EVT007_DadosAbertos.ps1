...
if ([string]::IsNullOrWhiteSpace($DataResultado)) { $DataResultado = [string]$exec.dataResultadoInicial }
if ($DataResultado -ne [string]$exec.dataResultadoInicial -or $DataResultado -ne [string]$exec.dataResultadoFinal) {
    throw "A consolidacao deve usar o mesmo D-1 definido em config\execucao\teste_d1.json: $($exec.dataResultadoInicial)."
}
